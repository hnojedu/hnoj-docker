...
inotifywait -rm "${problem_storage_root}" -e move,create,delete,close_write -q | while read -r line; do
    ...
    for judge in "${judges[@]}"; do
        curl --max-time 3 -4 -s -X POST "http://$judge/update/problems" | sed "s/^/$(date) [$judge]: /"';$a\'
    done
done